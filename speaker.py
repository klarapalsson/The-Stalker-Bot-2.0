...
# --- Internal state ---
_speech_queue = queue.Queue(maxsize=1)  # only keep latest pending message
_worker_thread = None
...
def _tts_worker():
    ...
    try:
        while not _worker_shutdown.is_set():
            try:
                message = _speech_queue.get(timeout=0.2)
            except queue.Empty:
                continue
...
def stop_engine():
    """Stop current pyttsx3 engine immediately."""
    with _engine_lock:
        eng = _engine
    if eng is not None:
        try:
...
def stop_tts(wait=True, timeout=1.0):
    """Signal worker to exit and optionally join."""
